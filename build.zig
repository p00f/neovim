...
pub fn build(b: *std.Build) !void {
    ...
    const utf8proc = if (use_system_utf8proc) null else b.lazyDependency("utf8proc", .{
        .target = target,
        .optimize = optimize,
    });
    const unibilium = if (use_unibilium and !use_system_unibilium) b.lazyDependency("unibilium", .{
        .target = target,
        .optimize = optimize,
    }) else null;
    // TODO(bfredl): fix upstream bugs with UBSAN
    const treesitter = if (use_system_treesitter) null else b.lazyDependency("treesitter", .{
        .target = target,
    ...
    if (use_system_lua) {
        const path = if (use_luajit)
            try getSystemIncludePath(b, "luajit")
        else
            try getSystemIncludePath(b, "lua5.1");
        try unittest_include_path.append(b.allocator, path);
    } else if (lua) |compile| {
        try unittest_include_path.append(b.allocator, compile.getEmittedIncludeTree());
    }
    if (use_system_uv) {
        try unittest_include_path.append(b.allocator, try getSystemIncludePath(b, "libuv"));
        try unittest_include_path.append(b.allocator, try getSystemIncludePath(b, "libluv"));
    } else {
        if (libuv) |compile| {
            try unittest_include_path.append(b.allocator, compile.getEmittedIncludeTree());
        }
    ...
    if (use_system_utf8proc) {
        try unittest_include_path.append(b.allocator, try getSystemIncludePath(b, "libutf8proc"));
    ...
    if (use_unibilium) {
        if (use_system_unibilium) {
            try unittest_include_path.append(b.allocator, try getSystemIncludePath(b, "unibilium"));
    ...
    if (use_system_treesitter) {
        try unittest_include_path.append(b.allocator, try getSystemIncludePath(b, "tree-sitter"));
    ...
    try build_lua.add_lua_modules(b, t, nvim_exe.root_module, lpeg, use_luajit, false);
    ...
    test_deps.dependOn(test_fixture(b, "tty-test", true, libuv, target, optimize, &flags));
    ...
    test_deps.dependOn(test_fixture(b, "streams-test", true, libuv, target, optimize, &flags));
    ...
    if (b.systemIntegrationOption("tree-sitter-c", .{})) {
        test_deps.dependOn(checkParser(b, "tree-sitter-c"));
    } else {
    ...
    if (b.systemIntegrationOption("tree-sitter-markdown", .{})) {
        test_deps.dependOn(checkParser(b, "tree-sitter-markdown"));
        test_deps.dependOn(checkParser(b, "tree-sitter-markdown-inline"));
    } else {
        if (b.lazyDependency("treesitter_markdown", .{
            .target = target,
            .optimize = optimize,
        })) |parser| {
            test_deps.dependOn(add_ts_parser(
                b,
                "markdown",
                parser.path("tree-sitter-markdown/"),
                true,
                target,
                optimize,
            ));
            test_deps.dependOn(add_ts_parser(
                b,
                "markdown_inline",
                parser.path("tree-sitter-markdown-inline/"),
                true,
                target,
                optimize,
            ));
    ...
    if (b.systemIntegrationOption("tree-sitter-vim", .{})) {
        test_deps.dependOn(checkParser(b, "tree-sitter-vim"));
    } else {
    ...
    if (b.systemIntegrationOption("tree-sitter-vimdoc", .{})) {
        test_deps.dependOn(checkParser(b, "tree-sitter-vimdoc"));
    } else {
    ...
    if (b.systemIntegrationOption("tree-sitter-lua", .{})) {
        test_deps.dependOn(checkParser(b, "tree-sitter-lua"));
    } else {
    ...
    if (b.systemIntegrationOption("tree-sitter-query", .{})) {
        test_deps.dependOn(checkParser(b, "tree-sitter-query"));
    } else {
...
pub fn test_fixture(
    ...
    fixture.linkLibC();
    if (use_libuv) {
        if (b.systemIntegrationOption("uv", .{})) {
...
fn checkParser(b: *std.Build, parser: []const u8) *std.Build.Step {
